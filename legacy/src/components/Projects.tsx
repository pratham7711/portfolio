...
const projects: Project[] = [
  ...
  },
  {
    ...
    name: 'MEGAKERNEL TTS',
    description:
      'Adapted AlpinDale\'s 1,200-line persistent CUDA megakernel as the talker-decode backend for Qwen3-TTS-1.7B on RTX 5090 Blackwell. Retuned kernel constants (hidden, intermediate, vocab, LM-head blocks, MAX_SEQ_LEN, rope_theta), implemented MRoPE for the multi-section rotary, and wired it into a streaming Pipecat voice loop with Groq LLM + Deepgram STT. TTS-side TTFC 25 ms, RTF 0.12 on the megakernel-backed decoder.',
    tags: ['CUDA', 'Python', 'PyTorch', 'Qwen3-TTS', 'Pipecat', 'Groq', 'Blackwell', 'Voice AI'],
    link: 'https://github.com/pratham7711/e3-megakernel-tts-takehome',
    loomUrl: 'https://www.loom.com/share/7768e549803c4a3a8678e4a5f39d996b',
    accent: '#76b900',
  },
]
...
export const Projects = () => {
  ...
  const goTo = useCallback(
    (index: number) => {
      if (isTransitioning || index === featuredIndex) return
      ...
      if (reduceMotion || !card) {
        tl.set({}, {}, 0.18)
          .add(() => setFeaturedIndex(index), 0.18)
          .to({}, { duration: 0.18 })
        return
      }

      const shards = burst ? Array.from(burst.querySelectorAll<HTMLSpanElement>(`.${styles.shard}`)) : []
      const accent = featuredProjects[index].accent ?? '#00E5D1'
      if (burst) burst.style.setProperty('--burst-accent', accent)

      // ── Outgoing: card implodes, shards fan outward, shockwave expands.
      tl.to(card, {
        scale: 0.92,
        rotation: -1.5,
        opacity: 0,
      ...
      if (shards.length) {
        tl.fromTo(
          ...
          {
            x: (i) => Math.cos((i / SHARD_COUNT) * Math.PI * 2) * 210,
            y: (i) => Math.sin((i / SHARD_COUNT) * Math.PI * 2) * 210,
            rotate: (i) => (i % 2 ? 140 : -140),
            scale: 0.35,
            opacity: 0,
            duration: 0.55,
            ease: 'power3.out',
      ...
      if (shock) {
        tl.fromTo(
          shock,
          { scale: 0.05, opacity: 0.55 },
          { scale: 1.9, opacity: 0, duration: 0.5, ease: 'power2.out' },
          0
        )
      }

      // ── Swap data midway (during the dimmest moment).
      tl.add(() => setFeaturedIndex(index), 0.24)

      // ── Incoming: card materializes with overshoot.
      tl.fromTo(
        card,
        { scale: 0.94, rotation: 1.5, opacity: 0 },
        {
          scale: 1,
          rotation: 0,
          opacity: 1,
      ...
      <section ref={featuredRef} id="work" className={styles.featured}>
        <div className={styles.featuredInner}>
              ...
              <div className={styles.navDots}>
                {featuredProjects.map((_, i) => (
                  <button
                    ...
                    style={i === featuredIndex ? { '--dot-accent': currentFeatured.accent } as React.CSSProperties : {}}
                  />
                ))}
              </div>
          ...
          <div className={`${styles.featuredVisual} featuredAnimIn`}>
            {/* Blowup overlay — shockwave ring + radial shards. pointer-events: none */}
            <div ref={burstRef} className={styles.cardBurst} aria-hidden="true">
              <svg className={styles.shockwave} viewBox="-50 -50 100 100">
                <circle ref={shockRef} cx="0" cy="0" r="42" />
              </svg>
